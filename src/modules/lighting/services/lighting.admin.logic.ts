...
export class LightingAdminService {
  ...
  async getCourtLightingConfig(courtId: string): Promise<LightingConfigResponseDto> {
  ...
  async updateCourtLightingConfig(
    ...
    if (payload.quietHoursStart && !LIGHTING_HH_MM_REGEX.test(payload.quietHoursStart)) {
    ...
    if (payload.quietHoursEnd && !LIGHTING_HH_MM_REGEX.test(payload.quietHoursEnd)) {
      throw new HttpException(
  ...
  async getDeviceStatus(
    ...
    return this.lightingOrchestratorService.getCourtDeviceStatus(courtId, refresh);
  }
